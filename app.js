...
function matchBatchesAgainstDeposits(batches, deposits, expectedDays, tolerance = 1) {
  ...
  for (const batch of batches) {
    ...
    if (found) {
      found.used = true;
      const lateDays = dayDiff(batch.date, found.date) - expectedDays >= 0
        ? dayDiff(batch.date, found.date)
        : dayDiff(batch.date, found.date);
      const isLate = dayDiff(batch.date, found.date) > expectedDays;
      results.push({
        expectedDate: batch.date,
        actualDate: found.date,
        lateDays: dayDiff(batch.date, found.date),
        amount: batch.amount,
        count: batch.count,
...
async function runReconciliation() {
  ...
  try {
    ...
    renderResults();
    switchView("results");
    if (window.FIREBASE_ENABLED) saveResultsToFirebase();
  } catch (err) {
...
function renderResults() {
  ...
  for (const r of rows) {
    ...
    tr.innerHTML = `
      <td><span class="tag-courier ${r.tag}">${r.label}</span></td>
...
function exportResultsExcel(rows, filename) {
  const data = [["ขนส่ง/ประเภท", "วันที่คาดว่าเงินเข้า", "วันที่เข้าจริง", "จำนวนวันช้า", "ยอดเงิน", "จำนวนรายการ", "สถานะ"]];
  const statusLabel = { ontime: "ตรงเวลา", late: "โอนช้า", missing: "ยังไม่พบ" };
  rows.forEach(r => {
    data.push([
      r.label, fmtDate(r.expectedDate), r.actualDate ? fmtDate(r.actualDate) : "-",
...
async function saveResultsToFirebase() {
  ...
  for (const r of lastResults) {
    const period = r.expectedDate.toISOString().slice(0, 7);
  ...
  for (const id in batch) {
    ...
    } catch (e) { console.error("save failed", id, e); }
  }
}
...
function buildCourierUploadGrid() {
  ...
  ["best", "jnt", "kerry"].forEach(key => {
    ...
    wireDropzone(`dz-${key}`, `file-${key}`, (files) => {
      uploaded.courierFiles[key] = Array.from(files);
      const dz = document.getElementById(`dz-${key}`);
      dz.classList.add("filled");
      const logEl = document.getElementById(`log-${key}`);
      logEl.style.display = "block";
      logEl.textContent = Array.from(files).map(f => "• " + f.name).join("\n");
    });
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  wireDropzone("dz-statement", "file-statement", (files) => {
    uploaded.statementFiles = Array.from(files);
    const dz = document.getElementById("dz-statement");
    dz.classList.add("filled");
    const listEl = document.getElementById("statementFileList");
