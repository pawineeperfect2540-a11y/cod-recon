...
const DEFAULT_HOLIDAYS_2026 = [
  ...
  "2026-10-16", // วันหยุดทำการเพิ่มเติมเป็นกรณีพิเศษ
...
function renderTwoSideSummary() {
  ...
  for (const acct in groups) {
    ...
    const deposits = (lastDepositsByAccount[acct] || []).slice().sort((a, b) => a.date - b.date);
    const missingBatches = lastResults.filter(r => normAcct(MAPPING[r.courierKey].account) === acct && r.status === "missing");
    ...
    html += `<div class="card" style="padding:0;overflow-x:auto;margin-bottom:14px;"><table>
      <thead><tr><th>วันที่</th><th>ยอดจากบัญชี</th><th>ตรง?</th><th>หมายเหตุ / ที่มา</th></tr></thead><tbody>`;
    if (!deposits.length) {
      html += `<tr><td colspan="4" style="color:var(--text-faint);">ยังไม่ได้อัปโหลด statement ของบัญชีนี้</td></tr>`;
    }
    for (const d of deposits) {
      ...
      html += `<tr>
        ...
        <td>${ok ? `ตรงกับ ${d.matchedLabel || "-"}` : (d.description ? d.description : "ไม่ทราบที่มา — ตรวจสอบเพิ่มเติม")}</td>
      </tr>`;
    }
...
function exportTwoSideExcel() {
  ...
  for (const acct in groups) {
    ...
    deposits.forEach(d => {
      data.push([fmtDate(d.date), d.amount, d.used ? "✓" : "✕", d.used ? `ตรงกับ ${d.matchedLabel || "-"}` : (d.description || "ไม่ทราบที่มา")]);
    });
    const missingBatches = lastResults.filter(r => normAcct(MAPPING[r.courierKey].account) === acct && r.status === "missing");
    if (missingBatches.length) {
      data.push([]);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("btnExportTwoSide").addEventListener("click", exportTwoSideExcel);

  document.querySelector('[data-view="twoside"]').addEventListener("click", renderTwoSideSummary);
